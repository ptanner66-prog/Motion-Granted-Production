...
MOTION SUMMARY:
- Type: ${state.orderContext.motionType}
- Case: ${state.orderContext.caseCaption}
- Case Number: ${state.orderContext.caseNumber}
- Jurisdiction: ${state.orderContext.jurisdiction}

QUALITY METRICS:
- Judge Simulation Grade: ${judgeResult?.grade || "N/A"}
- Citation Count: ${state.citationCount}
- Revision Loops: ${state.revisionLoopCount}

ACTION ITEMS:
1. Review the attached motion document
2. Verify all client-specific information
3. Sign where indicated
4. File according to local rules
5. Serve all parties per certificate of service

NOTES:
${judgeResult?.specificFeedback || "No specific notes"}
`;
...
Total Citations: ${state.citationCount}
Verification Status: ${state.citationCount >= CITATION_HARD_STOP_MINIMUM ? "PASSED" : "REVIEW NEEDED"}

Minimum Required: ${CITATION_HARD_STOP_MINIMUM}
Current Count: ${state.citationCount}

Generated: ${new Date().toISOString()}
`;

  // Generate Caption QC Report
  const captionQcReport = `
CAPTION QC REPORT
=================

Status: ${qcResult?.qcPasses ? "PASSED" : "ISSUES FOUND"}

${qcResult?.qcIssues?.length ? `Issues:\n${qcResult.qcIssues.join("\n")}` : "No issues found."}
...
`;
  ...
  await supabase.from("handoff_files").insert({
    order_workflow_id: state.workflowId,
    phase_number: 10,
    phase_name: "Final Assembly",
    handoff_type: "deliverables",
    content: {
      motion: finalDraft?.finalDraft,
  ...
  return {
    motionPdf: finalDraft?.finalDraft,
    attorneyInstructionSheet: attorneyInstructions,
    citationAccuracyReport: citationReport,
    captionQcReport: captionQcReport,
  };
}
...
/**
 * Main Inngest function for the 14-phase workflow orchestration
 */
export const generateOrderWorkflow = inngest.createFunction(
  {
    id: "generate-order-workflow",
    concurrency: {
      limit: 2, // Lower concurrency for complex workflow
    },
    retries: 3,
  },
  { event: "order/workflow-orchestrate" },
  async ({ event, step }) => {
    ...
    const workflowState = await step.run("initialize-workflow", async () => {
      ...
      // Gather full order context
      const contextResult = await gatherOrderContext(orderId);
      if (!contextResult.success || !contextResult.data) {
        throw new Error(`Failed to gather context: ${contextResult.error}`);
      }
      ...
      if (existingWorkflow) {
        ...
        // Get motion type ID
        const { data: motionType } = await supabase
          .from("motion_types")
          .select("id")
          .eq("code", order.motion_type)
          .single();
        ...
        const { data: newWorkflow, error: wfError } = await supabase
          ...
          .insert({
            order_id: orderId,
            motion_type_id: motionType?.id || null,
