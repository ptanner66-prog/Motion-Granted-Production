...
class MetricsCollector {
  ...
  async getWorkflowPerformance(orderId: string): Promise<WorkflowPerformance | null> {
    try {
      ...
      // Calculate totals
      const phaseDurations = metrics.filter((m) => m.metric_type === 'workflow_phase_duration');
      const apiLatencies = metrics.filter((m) => m.metric_type === 'api_call_latency');
      const revisions = metrics.filter((m) => m.metric_type === 'revision_loop');
      const citations = metrics.find((m) => m.metric_type === 'citation_verification');

      const totalWorkflowMetric = metrics.find((m) => m.metric_type === 'total_workflow_time');
      const totalDurationMs = totalWorkflowMetric?.metric_value ||
        phaseDurations.reduce((sum, m) => sum + m.metric_value, 0);

      return {
        orderId,
        totalDurationMs,
        phaseBreakdown: phaseDurations.map((m) => ({
        ...
        totalApiLatencyMs: apiLatencies.reduce((sum, m) => sum + m.metric_value, 0),
        revisionCount: revisions.length > 0 ? Math.max(...revisions.map((r) => r.metric_value)) : 0,
  ...
  async getSystemMetricsSummary(since: Date): Promise<{
    ...
    try {
      ...
      // Calculate averages
      const workflowTimes = workflowMetrics?.map((m) => m.metric_value) || [];
      const avgWorkflowTimeMs = workflowTimes.length > 0
        ? workflowTimes.reduce((a, b) => a + b, 0) / workflowTimes.length
        : 0;

      const apiLatencies = apiMetrics?.map((m) => m.metric_value) || [];
      ...
      return {
        totalOrders: new Set(workflowMetrics?.map((m) => m.order_id)).size,
        avgWorkflowTimeMs,
        avgApiLatencyMs,
