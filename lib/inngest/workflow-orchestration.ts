...
async function generateAndUploadInstructionSheet(
  ...
  try {
    ...
    // T-33: Build verificationSummary from CIV results
    const allVResults = [
      ...((state.phaseOutputs?.["V.1"] as Record<string, unknown>)?.verificationResults as Array<Record<string, unknown>> ?? []),
      ...((state.phaseOutputs?.["VII.1"] as Record<string, unknown>)?.verificationResults as Array<Record<string, unknown>> ?? []),
    ];
